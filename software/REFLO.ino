...
void setup() {
  ...
  SimbleeForMobile.deviceName = "REFLO";          // device name
...
void loop() {
  ...
  if(millis() - itime1 > 1000) { itime1 = millis();   // 1sec timer control
    StatusLED(); TCtemp = Rtemp(); Tdiff = SLIDER3 - TCtemp;
    if(SLIDER3 != 30) { // TARGET TEMP mode
    ...
    if(Autoreq) { automodeSetup(); Autoreq = 0; }     // selector change event 
